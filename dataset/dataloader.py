...
def get_files(root, mode):
    ...
    if mode == "test":
        ...
        for file in tqdm(all_images):
            ...
            # print(file.split("/")[4].split("\\"))
            labels.append(int(file.split("/")[4].split("\\")[0]))
...
def get_files_use(root, mode):
    # for test
    if mode == "test":
        # files = []
        # for img in os.listdir(root):
        #     files.append(root + img)
        files = pd.DataFrame({"filename": root}, index=[0])
        return files
    elif mode != "test":
        # for train and val
        all_data_path, labels = [], []
        ...
        for file in tqdm(all_images):
            all_data_path.append(file)
            labels.append(int(file.split("/")[3].split("\\")[0]))
        all_files = pd.DataFrame({"filename": all_data_path, "label": labels})
        return all_files
    else:
        print("check the mode please!")
