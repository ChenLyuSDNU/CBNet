...
def evaluate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            output = model(input)
            measure_inference_speed(model, (input,))
            loss = criterion(output, target)

            # 2.2.2 measure accuracy and record loss
            precision1, precision2 = accuracy(output, target, topk=(1, 3))
            # recall, precision, F1 = eval(output, target)
            # accuray = acc((target, output) * 100)
            # recall = rs((target, output) * 100)
            # precision = ps((target, output) * 100)
            # f1 = fs((target, output) * 100)
            # print('recall:', rs(target, output, average="macro") * 100)
            # print('precision:', ps(target, output, average="macro") * 100)
            # print('f1-score:', fs(target, output, average="macro") * 100)
            losses.update(loss.item(), input.size(0))
            top1.update(precision1[0], input.size(0))
            # recall.update(recall[0], input.size())
            # precision.update(precision[0], input.size())
            # F1.update(F1[0], input.size())
            # top3.update(precision2[0], input.size(0))

    return [losses.avg, top1.avg, top2.avg]
    # return [losses.avg, top1.avg, recall.avg, precision.avg, f1.avg]
...
def main():
    ...
    # test_files = get_files(config.test_data, "val")
    ...
    # 4.5.4 load dataset
    train_dataloader = DataLoader(ChaojieDataset(train_), batch_size=config.batch_size, shuffle=True,
    ...
    log.write(
        '                           |---------- VALID ---------|-------- TRAIN --------|------Accuracy------|------------|\n')
    log.write(
        'lr       iter     epoch    | loss   top-1   top-3     | loss   top-1    top-3       |    Current Best    | time       |\n')
    log.write(
        '-------------------------------------------------------------------------------------------------------------------------------\n')
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        for iter, (input, target) in enumerate(train_dataloader):
            ...
            # recall, precision, F1 = eval(output, target)
            # precision = ps(output, target)
            # recall = rs(output, target)
            # f1 = fs(output, target)
            train_losses.update(loss.item(), input.size(0))
            train_top1.update(precision1_train[0], input.size(0))
            # train_precision.update(precision[0], input.size(0))
            # train_recall.update(recall[0], input.size(0))
            # train_F1.update(F1[0], input.size(0))
            # train_f1.update(f1[0], input.size(0))
            # train_recall.update(recall[0], input.size(0))
            train_top3.update(precision2_train[0], input.size(0))
            ...
            print(
                '%0.4f %5.1f %6.1f        | %0.3f  %0.3f  %0.3f  |  %0.3f     %0.3f  %0.3f  |         %s         | %s' % ( \
                    ...
                    train_losses.avg, train_top1.avg, train_top3.avg, str(best_precision_save),
                    time_to_str((timer() - int(start)), 'min'))
                , end='', flush=True)
            # print(
            #     '%0.4f %5.1f %6.1f        | %0.3f  %0.3f  %0.3f         | %0.3f  %0.3f  %0.3f         |         %s         | %s' % ( \
            #         lr, iter / len(train_dataloader) + epoch, epoch,
        ...
        #     )
        log.write(
            '%0.4f %5.1f %6.1f        | %0.3f  %0.3f  %0.3f  | %0.3f  %0.3f  |         %s         | %s' % ( \
                lr, 0 + epoch, epoch,
                valid_loss[0], valid_loss[1], valid_loss[2],
                train_losses.avg, train_top1.avg,
                str(best_precision_save),
                time_to_str((timer() - start), 'min'))
        )
        log.write('\n')
        time.sleep(0.01)
